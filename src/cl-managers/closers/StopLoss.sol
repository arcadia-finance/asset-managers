...
import { IERC20 } from "../../cow-swapper/interfaces/IERC20.sol";
import { IFlashLoanRouter } from "../../../lib/flash-loan-router/src/interface/IFlashLoanRouter.sol";
import { IGPv2Settlement } from "../../cow-swapper/interfaces/IGPv2Settlement.sol";
import { IOrderHook } from "../../cow-swapper/interfaces/IOrderHook.sol";
...
/**
 * @title StopLosses for Arcadia Accounts.
 * @author Pragma Labs
 */
abstract contract StopLoss is IActionBase, AbstractBase, Guardian {
    ...
    address internal transient positionManager;
    // The id of the position.
    uint96 internal transient id;
    // The maximum amount of liquidity to decrease.
    uint128 internal transient liquidity;

    // The fee charged on the claimed fees of the liquidity position, with 18 decimals precision.
    uint256 internal transient claimFee;
    ...
    // The contract address of the numeraire (buyToken).
    address internal transient numeraire;
    ...
    function onSetAssetManager(address accountOwner_, bool, bytes calldata data) external {
        ...
        _setAccountInfo(msg.sender, accountOwner_, initiator_, maxClaimFee, maxSwapFee, orderHook, hookData, metaData_);
    }
    ...
    function setAccountInfo(
        ...
        _setAccountInfo(account_, accountOwner_, initiator_, maxClaimFee, maxSwapFee, orderHook, hookData, metaData_);
    }
    ...
    function _setAccountInfo(
        ...
        if (maxClaimFee > 1e18 || maxSwapFee > 1e18) revert InvalidValue();

        ownerToAccountToInitiator[accountOwner_][account_] = initiator_;
        // unsafe cast: maxClaimFee <= 1e18 < type(uint64).max.
        // forge-lint: disable-next-item(unsafe-typecast)
        accountInfo[account_] =
            AccountInfo({ maxClaimFee: uint64(maxClaimFee), maxSwapFee: uint64(maxSwapFee), orderHook: orderHook });
     ...
     * @param positionManager_ The contract address of the position manager.
     ...
     */
    function flashLoanAndCallBack(address account_, address positionManager_, uint256 id_, bytes calldata callBackData)
        external
        whenNotPaused
    {
        ...
        address numeraire_ = IAccount(account_).numeraire();
        ...
        uint256 balanceNumeraire = numeraire_.balanceOf(account_);
        bytes memory actionData = ArcadiaLogic._encodeAction(
            positionManager_,
            id_,
            numeraire,
            address(0),
            numeraire.balanceOf(account_),
            0,
            abi.encode(balanceNumeraire, callBackData)
        );
    ...
    function executeAction(bytes calldata actionTargetData) external override returns (ActionData memory depositData) {
        ...
        PositionState memory position = _getPositionState(positionManager_, id_);

        uint256 numeraireIndex = _getIndex(position.tokens, numeraire);
        uint256[] memory balances = new uint256[](position.tokens.length);
        balances[numeraireIndex] = balanceNumeraire;
        uint256[] memory fees = new uint256[](balances.length);

        // Claim pending yields and update balances.
        _claim(balances, fees, positionManager_, position, claimFee);

        // If the position is staked, unstake it.
        _unstake(balances, positionManager_, position);

        // Remove liquidity of the position and update balances.
        _burn(balances, positionManager_, position);
        ...
        tokenIn_.safeApproveWithRetry(VAULT_RELAYER, amountIn_);
        ...
        uint256 amount = IERC20(numeraire_).balanceOf(address(this));
        fee = amount.mulDivDown(swapFee, 1e18);
        amount = amount - fee;

        // Send the fee to the initiator.
        if (fee > 0) {
            address initiator_ = initiator;
            numeraire_.safeTransfer(initiator_, fee);
            emit FeePaid(msg.sender, initiator_, numeraire_, fee);
        }

        // Approve numeraire to be deposited into the account.
        numeraire_.safeApproveWithRetry(msg.sender, amount);
     ...
     * since the order hash and numeraire will not be set.
