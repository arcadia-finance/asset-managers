...
import { CollectParams, IncreaseLiquidityParams } from "../uniswap-v3/interfaces/INonfungiblePositionManager.sol";
 ...
 */
contract AlienBaseCompounder is IActionBase {
    ...
    // keccak256("AlienBaseCompounderAccount")
    bytes32 internal constant ACCOUNT_SLOT = 0x24f180efe883e327f853d7d785e971e11a0d28fedb8ee8907f0b25b8078882ab;
    // keccak256("AlienBaseCompounderTrustedSqrtPriceX96")
    bytes32 internal constant TRUSTED_SQRT_PRICE_X96_SLOT =
        0x991b078a4da81417937c4543d0278311ab0b4e897a30b286b821d32a6797fc8d;
    ...
    function executeAction(bytes calldata compoundData) external override returns (ActionData memory assetData) {
        ...
        // Collect fees.
        Fees memory fees;
        (fees.amount0, fees.amount1) = AlienBaseLogic.POSITION_MANAGER.collect(
            CollectParams({
                tokenId: id,
        ...
        ERC20(position.token0).safeApprove(address(AlienBaseLogic.POSITION_MANAGER), 0);
        ERC20(position.token0).safeApprove(address(AlienBaseLogic.POSITION_MANAGER), fees.amount0);
        ERC20(position.token1).safeApprove(address(AlienBaseLogic.POSITION_MANAGER), 0);
        ERC20(position.token1).safeApprove(address(AlienBaseLogic.POSITION_MANAGER), fees.amount1);
        AlienBaseLogic.POSITION_MANAGER.increaseLiquidity(
        ...
        AlienBaseLogic.POSITION_MANAGER.approve(msg.sender, id);
    }
        ...
        returns (bool zeroToOne, uint256 amountOut)
    {
        if (position.sqrtPriceX96 >= position.sqrtRatioUpper) {
            ...
            zeroToOne = true;
            amountOut = AlienBaseLogic._getAmountOut(position.sqrtPriceX96, true, fees.amount0);
            ...
            zeroToOne = false;
            amountOut = AlienBaseLogic._getAmountOut(position.sqrtPriceX96, false, fees.amount1);
        } else {
            // Position is in range.
            // Rebalance fees so that the ratio of the fee values matches with ratio of the position.
            uint256 targetRatio =
                AlienBaseLogic._getTargetRatio(position.sqrtPriceX96, position.sqrtRatioLower, position.sqrtRatioUpper);

            // Calculate the total fee value in token1 equivalent:
            uint256 fee0ValueInToken1 = AlienBaseLogic._getAmountOut(position.sqrtPriceX96, true, fees.amount0);
            ...
            if (currentRatio < targetRatio) {
                ...
                amountOut = AlienBaseLogic._getAmountOut(position.sqrtPriceX96, false, amountIn);
            }
        }
    }

    /**
     * @notice Swaps one token to the other token in the Alien Base Pool of the Liquidity Position.
     * @param position Struct with the position data.
    ...
    function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external {
        // Check that callback came from an actual Alien Base pool.
        (address token0, address token1, uint24 fee) = abi.decode(data, (address, address, uint24));
        if (AlienBaseLogic._computePoolAddress(token0, token1, fee) != msg.sender) revert OnlyPool();
