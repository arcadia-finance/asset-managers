...
/**
 * @notice Fuzz tests for the function "_approveAndTransfer" of contract "Closer".
 */
contract ApproveAndTransfer_Closer_Fuzz_Test is Closer_Fuzz_Test {
    /* ///////////////////////////////////////////////////////////////
                              SETUP
    /////////////////////////////////////////////////////////////// */

    function setUp() public override(Closer_Fuzz_Test) {
    ...
    function testFuzz_Success_approveAndTransfer_AllZero(
        ...
        vm.assume(account_ != address(closer));

        // And: The initiator is not the contract.
        vm.assume(initiator != address(closer));

        // And: The position id is 0.
        position.id = 0;
        ...
        deal(address(token0), address(closer), balance0, true);
        deal(address(token1), address(closer), balance1, true);

        // And: Uniswap v3 position.
        nonfungiblePositionManager.mint(address(closer), position.id);
        ...
        uint256 count;
        (count, balances) =
            closer.approveAndTransfer(initiator, balances, fees, address(nonfungiblePositionManager), position);

        // Then: It should return the correct count (0 because position.id = 0 and no leftovers).
        assertEq(count, 0);

        // And: ERC20 tokens are approved.
        assertEq(balances[0], 0);
        assertEq(balances[1], 0);
        assertEq(token0.allowance(address(closer), account_), 0);
        assertEq(token1.allowance(address(closer), account_), 0);
    ...
    function testFuzz_Success_approveAndTransfer_TokensZero(
        address account_,
        PositionState memory position,
        uint256 balance0,
        uint256 balance1,
        ...
        // And: The position id is non-zero.
        position.id = bound(position.id, 1, type(uint256).max);
    ...
    function testFuzz_Success_approveAndTransfer_Token1Zero(
        ...
        // Then: It should return the correct count (1 for position + 1 for token0 leftover).
        assertEq(count, 2);
        ...
        assertEq(balances[1], 0);
        assertEq(token0.allowance(address(closer), account_), balances[0]);
        assertEq(token1.allowance(address(closer), account_), 0);
    ...
    function testFuzz_Success_approveAndTransfer_Token0Zero(
        ...
        // Then: It should return the correct count (0 for position + 1 for token1 leftover).
        assertEq(count, 1);

        // And: The position is not approved (position.id = 0).
        assertEq(ERC721(address(nonfungiblePositionManager)).getApproved(position.id), address(0));

        // And: ERC20 tokens are approved.
        assertEq(balances[0], 0);
        assertEq(balances[1], balance1 - fee1);
        assertEq(token0.allowance(address(closer), account_), 0);
        assertEq(token1.allowance(address(closer), account_), balances[1]);
    ...
    function testFuzz_Success_approveAndTransfer_IdZero(
        address account_,
        PositionState memory position,
        uint256 balance0,
        uint256 balance1,
        ...
        // Then: It should return the correct count (0 for position + 1 for token0 + 1 for token1).
        assertEq(count, 2);
    ...
    function testFuzz_Success_approveAndTransfer_AllNonZero(
        ...
        // Then: It should return the correct count (1 for position + 1 for token0 + 1 for token1).
        assertEq(count, 3);
