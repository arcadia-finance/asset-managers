...
contract GetTargetRatio_RebalanceLogic_Fuzz_Test is RebalanceLogic_Fuzz_Test {
    ...
    function testFuzz_Revert_getTargetRatio_OverflowPriceX96(
        ...
        sqrtPriceX96 = bound(sqrtPriceX96, uint256(type(uint128).max) + 1, TickMath.MAX_SQRT_RATIO - 1);

        // And: sqrtRatioLower is smaller than sqrtPriceX96.
        sqrtRatioLower = bound(sqrtRatioLower, TickMath.MIN_SQRT_RATIO, sqrtPriceX96 - 1);

        // And: sqrtRatioUpper is bigger than sqrtPriceX96.
        sqrtRatioUpper = bound(sqrtRatioUpper, sqrtPriceX96 + 1, TickMath.MAX_SQRT_RATIO);
        ...
        public
    {
        ...
        sqrtPriceX96 = bound(sqrtPriceX96, TickMath.MIN_SQRT_RATIO + 1, uint256(type(uint128).max));
